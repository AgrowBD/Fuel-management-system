"use client";
// Admin government quota rules page: view and edit fuel quota rules for government vehicles.
...
export default function AdminGovtRulesPage() {
  const [rules, setRules] = useState<Rule[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/admin/govt-rules")
      .then((r) => r.json())
      .then(setRules)
  ...
  async function handleSave(vehicleType: string, data: Partial<Rule>) {
    const res = await fetch(`/api/admin/govt-rules/${vehicleType}`, {
      method: "PATCH",
    ...
    if (res.ok) {
      ...
      toast.success("Government quota rule updated");
    } else {
  ...
  return (
    <div className="flex flex-col h-full">
      <Topbar title="Government Quota Rules" />
      <div className="flex-1 p-4 md:p-6 space-y-4">
        <div className="flex items-center gap-2">
          <Badge className="bg-blue-600 text-white">Government Only</Badge>
          <p className="text-sm text-muted-foreground">
            These quotas apply exclusively to vehicles flagged as government vehicles. Higher limits reflect official operational requirements.
          </p>
        </div>
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Government Vehicle Fuel Quotas</CardTitle>
            <p className="text-sm text-muted-foreground">
              Edit max liters and restriction period per vehicle type. Changes take effect immediately for new transactions.
            </p>
          </CardHeader>
          <CardContent>
            {loading ? (
